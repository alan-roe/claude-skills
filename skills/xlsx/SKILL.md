...
- **`recalc.py` auto-adds the `_xlfn.` prefix** (and the equivalent prefix for ~50 other future functions — math/trig, bit, date, and text functions that don't spill) before recalculating, so writing these bare also works; the prefix only matters if you inspect the raw formula string yourself. It deliberately leaves `XLOOKUP`, `XMATCH`, `SORT`, `FILTER`, `UNIQUE`, and `SEQUENCE` untouched — see below.
