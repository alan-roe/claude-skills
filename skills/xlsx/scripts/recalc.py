...
def _recalc_with_profile(filename, abs_path, timeout, profile_dir: Path):
    ...
    # Fix _xlfn. prefixes for OOXML future functions (MAXIFS, IFS, etc.) so
    # LibreOffice doesn't show #NAME? errors. Must run before the `before`
    # stamp below, or a fix-triggered rewrite would masquerade as soffice's.
    try:
